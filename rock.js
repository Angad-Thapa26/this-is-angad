...
const $winCounter = document.getElementById('winCounter'); // Win counter element
...
let state = '';
let botArmy = '';
let playerWins = 0; // Win counter variable
...
const updateWinCounter = () => {
	$winCounter.innerText = `Your Wins: ${playerWins}`;
};
...
$start.addEventListener('click', () => {
	...
	setTimeout(() => {
		...
		if (state === 'WIN') {
			playerWins += 1; // Increment the win counter
			updateWinCounter(); // Update the win counter display
			confetti(); // Trigger confetti if player wins
		}
	}, 2500);
});
